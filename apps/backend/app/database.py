...
class Database:
    ...
    @staticmethod
    def _careerpilot_profile_to_dict(row: CareerPilotProfile) -> dict[str, Any]:
        return {
    ...
    async def get_careerpilot_profile(self) -> dict[str, Any] | None:
        async with self._session() as session:
            row = await session.get(CareerPilotProfile, "default")
            return self._careerpilot_profile_to_dict(row) if row else None

    async def upsert_careerpilot_profile(self, values: dict[str, Any]) -> dict[str, Any]:
        async with self._write_session() as session:
            row = await session.get(CareerPilotProfile, "default")
            now = _now()
            if row is None:
                row = CareerPilotProfile(profile_id="default", created_at=now, updated_at=now, **values)
                session.add(row)
            else:
            ...
            return self._careerpilot_profile_to_dict(row)
    ...
    def _job_to_dict(row: Job) -> dict[str, Any]:
        doc: dict[str, Any] = {
            ...
            "skills_preferred": row.skills_preferred,
    ...
    def _application_to_dict(row: Application) -> dict[str, Any]:
        ...
        if isinstance(row.metadata_json, dict):
            result.update(row.metadata_json)
        return result

    @staticmethod
    def _application_event_to_dict(row: ApplicationEvent) -> dict[str, Any]:
        return {
    ...
    @staticmethod
    def _careerpilot_job_values(job: dict[str, Any]) -> dict[str, Any]:
        """Build ORM values and legacy metadata for a normalized job."""
        job_id = str(job.get("id") or job.get("job_id") or uuid4())
        discovered_at = job.get("discovered_at") or _now()
        return {
            "job_id": job_id,
            "content": str(job.get("description") or ""),
            "resume_id": job.get("resume_id"),
            "external_id": job.get("external_id") or job.get("id"),
            "provider": job.get("provider") or "demo",
            ...
            "metadata_json": {
                ...
                "discovered_at": discovered_at,
                "provider_metadata": job.get("provider_metadata") or {},
                "screening_questions": job.get("screening_questions") or [],
            },
        }

    @staticmethod
    def _apply_careerpilot_job(row: Job, normalized: dict[str, Any]) -> None:
        """Apply normalized values to an existing or new ORM row."""
        for key, value in normalized.items():
            if key in {"job_id", "metadata_json"}:
                continue
            setattr(row, key, value)
        row.metadata_json = normalized["metadata_json"]

    async def upsert_careerpilot_job(self, job: dict[str, Any]) -> dict[str, Any]:
        """Insert or update a normalized CareerPilot job asynchronously."""
        normalized = self._careerpilot_job_values(job)
        ...
        async with self._write_session() as session:
            ...
            if row is None:
                ...
                self._apply_careerpilot_job(row, normalized)
            await session.commit()
            return self._job_to_dict(row)

    def upsert_careerpilot_job_sync(self, job: dict[str, Any]) -> dict[str, Any]:
        """Insert or update a normalized CareerPilot job synchronously."""
        normalized = self._careerpilot_job_values(job)
        with self._sync_write_session() as session:
            row = session.get(Job, normalized["job_id"])
            if row is None:
                row = Job(**normalized)
                session.add(row)
            else:
    ...
    async def list_careerpilot_jobs(self, limit: int = 50) -> list[dict[str, Any]]:
        """List normalized CareerPilot jobs ordered by discovered_at descending."""
        async with self._session() as session:
            result = await session.execute(
                select(Job)
                .where(Job.provider.is_not(None))
                .order_by(Job.discovered_at.desc().nullslast(), Job.created_at.desc())
                .limit(limit)
            )
    ...
    async def create_application(
        ...
        async with self._write_session() as session:
            ...
            row.metadata_json = copy.deepcopy(metadata or {})
            try:
                await session.commit()
    ...
    async def update_application_metadata(
        ...
        """Merge CareerPilot draft fields without changing tracker semantics."""
        async with self._write_session() as session:
    ...
    async def find_careerpilot_application(
        self, job_id: str, source_resume_id: str
    ) -> dict[str, Any] | None:
        """Find a draft by source job and original resume."""
        async with self._session() as session:
            result = await session.execute(select(Application).where(Application.job_id == job_id))
            for row in result.scalars().all():
                metadata = row.metadata_json or {}
                if metadata.get("source_resume_id") == source_resume_id:
                    return self._application_to_dict(row)
        return None
    ...
    async def create_application_event(
        ...
        """Append one immutable application lifecycle event."""
        ...
        async with self._write_session() as session:
            session.add(row)
            await session.commit()
        return self._application_event_to_dict(row)
    ...
    async def list_application_events(self, application_id: str) -> list[dict[str, Any]]:
        ...
        async with self._session() as session:
            result = await session.execute(
                ...
                .order_by(ApplicationEvent.timestamp, ApplicationEvent.event_id)
            )
            return [self._application_event_to_dict(row) for row in result.scalars().all()]
    ...
    async def reset_database(self) -> None:
        ...
        async with self._write_session() as session:
            ...
            await session.execute(delete(CareerPilotProfile))
            await session.commit()
