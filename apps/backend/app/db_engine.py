...
def init_models_sync(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        ...
        job_columns = conn.exec_driver_sql("PRAGMA table_info(jobs)").mappings().all()
        existing_job_columns = {column["name"] for column in job_columns}
        careerpilot_job_columns = {
            "external_id": "TEXT",
            "provider": "TEXT",
            "company": "TEXT",
            "title": "TEXT",
        ...
        if job_columns:
            for name, sql_type in careerpilot_job_columns.items():
        ...
        if application_columns and "metadata_json" not in {column["name"] for column in application_columns}:
            conn.exec_driver_sql("ALTER TABLE applications ADD COLUMN metadata_json JSON")
